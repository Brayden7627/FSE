...
def mainMenu():
    ...
    # --- BUG SETUP VARIABLES ---
    # These just look like regular coordinate trackers
    drift_x = 0
    drift_y = 0
    ...
    while running:
        ...
        # --- THE BUG ---
        # Slowly increment the drift values every frame
        drift_x += 0.25 
        drift_y += 0.15 
        
        # Calculate the "ghost" mouse position
        ghost_x = mx - int(drift_x)
        ghost_y = my - int(drift_y)
        # ---------------
        ...
        for evt in event.get():
            ...
            if evt.type==MOUSEBUTTONDOWN:
                if evt.button==1:  
                    # Checking collision against the ghost coordinates, not the real mouse
                    if menuButtons[1].collidepoint(ghost_x, ghost_y):
                        return "level1"
