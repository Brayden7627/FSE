...
while time.get_ticks() - loadingStart < 5000:
    for e in event.get():
        if e.type == QUIT:
            quit()
...
def mainMenu():
    c = time.Clock()
    btnMenu         = Rect(100, 100, 200, 50)
    btnStart        = Rect(100, 200, 200, 50)
    btnStory        = Rect(100, 300, 200, 50)
    btnInstructions = Rect(100, 400, 200, 50)
    ...
    while True:
        ...
        for btn in [btnMenu, btnStart, btnStory, btnInstructions]:
            draw.rect(screen, blue, btn)

        screen.blit(f.render("MENU",         True, white), (160, 110))
        screen.blit(f.render("START",        True, white), (155, 210))
        screen.blit(f.render("STORY",        True, white), (155, 310))
        screen.blit(f.render("INSTRUCTIONS", True, white), (108, 410))
