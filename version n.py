...
def render(p, cam, coin_count, lives, invincibility, coins, enemies, fishes, bombers, projectiles, dropped_coins, is_rolling, elapsed_time, win_box, jump_pad, monitor, star, power_timer):
    ...
    if should_draw:
        if is_rolling == True:
            ...
            draw.circle(screen, ball_color, (draw_x, draw_y), radius)
            ...
            # Safe alpha blending technique to draw a golden tint without crashing
            if god_mode and (power_timer % 4 < 2):
                tint_surf = Surface((SPRITE_W, SPRITE_H), SRCALPHA)
                tint_surf.fill((255, 255, 0, 140)) # Translucent yellow layer
