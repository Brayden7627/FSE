...
spike_colour   = (249, 245, 241)
jumppad_colour = (73, 73, 0)

# ── Jump pad ─────────────────────────────────────────────────────────────────
def check_jump_pad(p, cam, vel, is_jumping):
    """Colour-detect jump pad directly under Sonic's feet and launch him."""
    foot_y = int(p[Y] + p[H] - cam[1])

    sample_xs = [
        int(p[X] - cam[0] + p[W] * 0.25),
        int(p[X] - cam[0] + p[W] * 0.5),
        int(p[X] - cam[0] + p[W] * 0.75),
    ]

    for sx in sample_xs:
        if 0 <= sx < w and 0 <= foot_y < h:
            c = screen.get_at((sx, foot_y))[:3]
            if all(abs(c[i] - jumppad_colour[i]) <= 15 for i in range(3)):
                vel[1] = -42          # big upward launch
                is_jumping = True
...
# ── Spikes ───────────────────────────────────────────────────────────────────
def check_spikes(p, cam, vel, invincibility, coin_count, lives, dropped_coins):
    if invincibility > 0:
    ...
    for sx in sample_xs:
        if 0 <= sx < w and 0 <= foot_y < h:
            c = screen.get_at((sx, foot_y))[:3]
            if all(abs(c[i] - spike_colour[i]) <= 15 for i in range(3)):
                hit = True
                break

    if hit:
        vel[1] = -12
        vel[0] = 10 if vel[0] >= 0 else -10
        invincibility = 120
...
def play_level():
    ...
    while True:
        ...
        # Colour checks after render so screen pixels are ready
        invincibility, coin_count, lives = check_spikes(p, cam, vel, invincibility, coin_count, lives, dropped_coins)
        is_jumping = check_jump_pad(p, cam, vel, is_jumping)
